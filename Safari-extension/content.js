...
function scrapeIndeed() {
  const selectors = {
    ...
    company: [
      ...
      '.css-1saizt3 a',
      'h2[data-testid="jobsearch-CompanyInfoContainer"] a',
    ],
    location: [
      '[data-testid="job-location"]',
  ...
  let company = findFirst(selectors.company);
  ...
  if (!company) {
    try {
      ...
      for (const script of scripts) {
        ...
        const org = data?.hiringOrganization?.name || data?.employerOverview?.name;
        if (org) { company = org; break; }
      }
    } catch (e) { /* ignore */ }
  }
