"""
Meeting prep orchestration helpers.
"""
...
def _summarise_article(
    ...
    """
    Use OpenAI to craft a 30–40 word summary that is meeting ready.
    """
    ...
    prompt = (
        "You are preparing a high-quality meeting brief. Surface ONLY information that is accurate, clearly relevant, and natural for conversation.\n\n"
        "NEWS CONTENT RULES:\n"
    ...
    try:
        ...
        print(f"[Meeting] Summary generation failed: {exc}")
...
def _generate_industry_overview(
    ...
    try:
        ...
        print(f"[Meeting] Industry summary failed: {exc}")
        # Return empty string to maintain quality standards (no generic fallback)
        return ""
    ...
    *,
    ...
    try:
        ...
        if result.get("content"):
            ...
            logger.info("Perplexity quick_search succeeded for meeting research")
    ...
    for query, query_type in queries:
        ...
        if not query:
            continue
        print(f"[Meeting] SERP query ({query_type}): {query}")
        ...
        try:
            ...
            print(f"[Meeting] SERP request failed: {exc}")
            continue
...
def fetch_comprehensive_research(
    ...
    try:
        ...
        query = (
            "Comprehensive research for a meeting: "
            + ", ".join(query_parts)
...
def infer_hometown_from_education(education: Sequence[str], contact_data: Optional[Dict[str, Any]] = None) -> Optional[str]:
    ...
    for edu_entry in education:
        ...
        if match:
            ...
            print(f"[Meeting] Found hometown in education: {hometown}")
            return hometown
        ...
        if city and state:
            ...
            if (city_clean[0].isupper() if city_clean else False) and len(state_clean) == 2 and state_clean.isalpha():
                ...
                if has_high_school:
                    ...
                    print(f"[Meeting] Found hometown from location fields (high school match): {hometown}")
                    return hometown

                # No high school but PDL has location - still useful as a conversation hook
                hometown = f"{city_clean}, {state_clean}"
                print(f"[Meeting] Using PDL location as hometown: {hometown}")
                return hometown

    # No explicit hometown found
    print(f"[Meeting] No explicit hometown found in education or location fields")
    return ""
