...
def submit_auto_apply(job_id: str):
    ...
    # Body extracted to submit_service.submit_auto_apply_for_user so Scout's
    # chat tool and this route share one implementation of the eligibility /
    # profile / credit checks and the worker spawn.
    ...
    result, status = submit_auto_apply_for_user(
        uid, str(job_id), dry_run=dry_run, edited_answers=edited_answers,
    )
