...
from app.config import MEETING_CREDITS, TIER_CONFIGS
...
from app.utils.validation import MeetingPrepRequest, validate_request
...
from app.services.pdf_builder import generate_meeting_pdf_v2
from app.utils.meeting_prep import (
...
from app.utils.users import (
    parse_resume_info,
    build_meeting_user_context,
    _empty_meeting_user_context,
)

logger = logging.getLogger(__name__)

meeting_bp = Blueprint(
    "meeting_prep", __name__, url_prefix="/api/meeting-prep"
)
...
def process_meeting_prep_background(
    prep_id,
    linkedin_url,
    user_id,
    resume_text,
    ...
    """Background worker to process meeting prep.
    ...
    try:
        ...
        if not contact_data:
            ...
            refund_credits_atomic(user_id, credits_charged, "meeting_enrichment_failed")
            prep_ref.update(
                {
                    "status": "failed",
        ...
        if resume_text and len(resume_text.strip()) > 50:
            ...
            user_context = build_meeting_user_context(parsed_resume, user_profile)
        else:
            # Build minimal context from profile
            user_context = _empty_meeting_user_context()
            if user_profile:
                resume_parsed = user_profile.get("resumeParsed")
                if resume_parsed and isinstance(resume_parsed, dict):
                    user_context = build_meeting_user_context(resume_parsed, user_profile)
                else:
                    user_context["name"] = (
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            f_similarity = executor.submit(
                generate_meeting_similarity, contact_data, user_context, research)
            f_questions = executor.submit(
                generate_meeting_questions, contact_data, user_context, research)
        ...
        pdf_buffer = generate_meeting_pdf_v2(
        ...
        logger.error(f"Meeting prep failed for {prep_id}: {e}")
        traceback.print_exc()

        # Refund credits on failure
        try:
            success, _ = refund_credits_atomic(user_id, credits_charged, "meeting_prep_failure")
            if success:
...
def create_meeting_prep():
    ...
    if PDL_OUTAGE_ACTIVE:
        return jsonify({"error": "service_unavailable", "message": "Meeting Prep temporarily unavailable due to a data provider update.", "code": "PDL_OUTAGE"}), 503
    try:
        print("\n=== MEETING PREP START ===")
        db = get_db()
        ...
        try:
            validated_data = validate_request(MeetingPrepRequest, data)
        ...
        if db and user_id:
            ...
            if user_doc.exists:
                ...
                # Check credits
                if credits_available < MEETING_CREDITS:
                    return (
                        jsonify(
                            {
                                "error": f"Insufficient credits. You need {MEETING_CREDITS} credits.",
                                "credits_needed": MEETING_CREDITS,
                ...
                # Check feature access (meeting prep limit)
                tier = user_data.get("subscriptionTier") or user_data.get("tier", "free")
                tier_config = TIER_CONFIGS.get(tier, TIER_CONFIGS["free"])
                allowed, reason = can_access_feature(tier, "meeting_prep", user_data, tier_config)

                if not allowed:
                    meeting_limit = tier_config.get("coffee_chat_preps", 0)
                    if meeting_limit != "unlimited":
                        current_usage = user_data.get("coffeeChatPrepsUsed", 0)
                        raise AuthorizationError(
                            f"Meeting Prep limit reached. You've used {current_usage} of {meeting_limit} allowed.",
                            details={
        ...
        success, new_balance = deduct_credits_atomic(user_id, MEETING_CREDITS, "meeting_prep")
        if not success:
            return (
                jsonify(
                    {
        ...
        try:
            thread = threading.Thread(
                target=process_meeting_prep_background,
                args=(
                    prep_id,
                    linkedin_url,
                    user_id,
            ...
            # Log meeting_prep_used metric (manual trigger)
            from app.utils.metrics_events import log_event
            log_event(user_id, "meeting_prep_used", {
...
@meeting_bp.route("/history", methods=["GET"])
@require_firebase_auth
def get_meeting_history():
    """Get recent meeting prep history"""
    try:
        db = get_db()
        user_id = request.firebase_user.get("uid")
...
def get_all_coffee_chat_preps():
    ...
    try:
        ...
        logger.error(f"Error getting all meeting preps: {e}")
        traceback.print_exc()
        return jsonify({"preps": [], "error": "Failed to load meeting preps"}), 200


@meeting_bp.route("/<prep_id>/download", methods=["GET"])
@require_firebase_auth
def download_meeting_pdf(prep_id):
    """Return Meeting PDF download URL"""
    try:
        db = get_db()
        user_id = request.firebase_user.get("uid")
        prep_ref = (
...
@meeting_bp.route("/<prep_id>", methods=["GET"])
@require_firebase_auth
def get_meeting_prep(prep_id):
...
@meeting_bp.route("/<prep_id>", methods=["DELETE"])
@require_firebase_auth
def delete_meeting_prep(prep_id):
    """Delete a meeting prep"""
    try:
        db = get_db()
        user_id = request.firebase_user.get("uid")

        print(f"[MeetingPrep] DELETE request for prep_id: {prep_id}")
        ...
        print(f"[MeetingPrep] Prep exists: {prep_doc.exists}")
