...
- Workflow chip contract: execute -> result in chat -> ONE navigate chip. `_enrich_workflow_ctas` deterministically attaches the fallback chip (Applications after a queued auto-apply, Job Board with query prefill after job matches, Open-your-network after a find, Find-people-at-X after intel) when the model forgets; model-authored ctas win. CTA chips whose route carries non-`tab` query params always navigate even when already on the page (fixes /outbox?contact= too).
- Still page-driven (not yet in-chat): hiring-manager search (no completion event dispatch from RecruiterSpreadsheetPage) and cover letters. Loops setup stays a page flow deliberately.
